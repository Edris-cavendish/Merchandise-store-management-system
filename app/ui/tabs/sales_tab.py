...
class SalesTab(ScrollablePage):
    ...
    def _build_product_panel(self) -> None:
        self.catalog_panel = ttk.LabelFrame(self.main_panel, text="Point of Sale Catalog", padding=14)
        ...
        ttk.Label(self.catalog_panel, text="Product", style="FormLabel.TLabel").grid(row=0, column=0, sticky="w", pady=(0, 4))
        ttk.Label(self.catalog_panel, text="Quantity", style="FormLabel.TLabel").grid(row=0, column=1, sticky="w", pady=(0, 4))

        self.product_box = ttk.Combobox(self.catalog_panel, textvariable=self.product_var, state="readonly")
        self.product_box.grid(row=1, column=0, sticky="ew", padx=(0, 10), pady=(0, 10))
        ttk.Entry(self.catalog_panel, textvariable=self.quantity_var, width=12).grid(
            row=1, column=1, sticky="ew", pady=(0, 10)
        )
        ttk.Button(self.catalog_panel, text="Add to Cart", style="Primary.TButton", command=self._add_to_cart).grid(
            row=1, column=2, sticky="ew", padx=(10, 0), pady=(0, 10)
        )
        self.catalog_panel.columnconfigure(2, weight=1)
        ...
        cart_x.grid(row=3, column=0, columnspan=3, sticky="ew", pady=(8, 0))
    ...
    def _build_checkout_panel(self) -> None:
        ...
        ttk.Label(self.checkout_panel, textvariable=self.store_name_var, style="Section.TLabel").grid(row=0, column=0, sticky="w")
        ttk.Label(
            self.checkout_panel,
            text="Award-winning POS layout with fast totals, tax, discounts, and exportable receipts.",
            style="Muted.TLabel",
            wraplength=420,
        ...
        make_labeled_entry(form, "Tax Rate", self.tax_var, 0, 1)
        ...
        summary = ttk.Frame(self.checkout_panel, style="Card.TFrame", padding=18)
        summary.grid(row=4, column=0, sticky="ew", pady=(12, 16))
        ...
        rows = (
            ...
            ("Tax", self.tax_amount_var),
            ("Total", self.total_var),
        )
        for row_index, (label, variable) in enumerate(rows):
            ttk.Label(summary, text=label, style="CardTitle.TLabel").grid(row=row_index, column=0, sticky="w", pady=4)
            ttk.Label(summary, textvariable=variable, style="CardValue.TLabel").grid(
                row=row_index, column=1, sticky="e", pady=4
            )

        ttk.Button(self.checkout_panel, text="Complete Sale", style="Primary.TButton", command=self._checkout).grid(
            row=5, column=0, sticky="ew"
        )
        ttk.Button(self.checkout_panel, text="Save Receipt", style="Secondary.TButton", command=self._save_receipt).grid(
            row=6, column=0, sticky="ew", pady=(10, 16)
        )

        ttk.Label(self.checkout_panel, text="Receipt Preview", style="Section.TLabel").grid(row=7, column=0, sticky="w")
        ...
        preview_x.grid(row=9, column=0, sticky="ew", pady=(8, 0))
